...
1. **設定画面**（`/settings`）で API Key・配信 URL（複数可。1 行に 1 つ、最大 10 件）・App Token・サブコンポジション名・フィールド ID を入力して保存します。
...
| 項目 | 内容 |
|---|---|
| 取得するコメント | スパチャ／メンバーのコメント／通常コメントを組み合わせて選択（取得と一覧表示の対象） |
...
- 複数の配信から同時に取得する場合は、上限を配信の数で分け合います（2 配信なら間隔も約 2 倍）。

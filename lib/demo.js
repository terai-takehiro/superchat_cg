...
const { msUntilReset } = require('./quota');
...
class DemoChat extends EventEmitter {
  ...
  status() {
    return {
      running: this.running, videoId: 'demo', title: `【デモ】ダミー配信（${this.video || 'demo'}）`, channelTitle: 'デモ', lastError: '',
      intervalMs: this.running ? 2000 : 0, pausedUntil: 0, quotaUsed: this.seq * 5, dailyQuota: 10000, resetAt: Date.now() + msUntilReset(),
    };
  }
  ...
  _make() {
    ...
    const msg = {
      id: `demo-${this.video || ''}-${Date.now()}-${++this.seq}`,

...
const DEFAULTS = {
  ...
  youtube: {
    ...
    source: 'web',
    apiKey: '',
    // 配信の URL または動画 ID（複数の配信から同時に取得できる）
    videos: [],
...
function merge(base, override) {
  ...
  for (const key of Object.keys(base)) {
    ...
    if (Array.isArray(b)) {
      if (Array.isArray(o)) out[key] = o.map((v) => String(v ?? '').trim()).filter(Boolean);
...
function load() {
  try {
    ...
    // 旧設定（配信 1 つ）からの引き継ぎ
    if (raw.youtube && !raw.youtube.videos && raw.youtube.video) raw.youtube.videos = [raw.youtube.video];

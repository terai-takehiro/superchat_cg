...
// チャンネル独自の絵文字（:_name:）。YouTube 上は画像なので、CG の文字としては意味をなさない
const CUSTOM_EMOJI = /:_[^:\s]+:/g;

function cleanComment(text, customEmoji) {
  if (customEmoji === 'keep') return text;
  return text.replace(CUSTOM_EMOJI, '').replace(/[ \u3000]{2,}/g, ' ').trim();
}

function buildPayload(msg, fields, { customEmoji = 'remove' } = {}) {
  const payload = {};
  ...
  put(fields.comment, cleanComment(msg.comment || '', customEmoji));
...
class CgController extends EventEmitter {
  ...
  async _show(msg, { update = false } = {}) {
    ...
    try {
      ...
      await this._control([{ ...target, payload: buildPayload(msg, s.fields, s) }]);
...
module.exports = { CgController, testConnection, buildPayload, cleanComment };

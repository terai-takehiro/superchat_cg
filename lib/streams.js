...
class StreamManager extends EventEmitter {
  ...
  // 同じ URL のものは使い回し、取り込み済みの記録を残す（再開時に同じコメントを二重に取り込まないため）。
  // force = true（取得方法を変えたときなど）はすべて新しく作る
  rebuild(videos, { force = false } = {}) {
    this.stopAll();
    const prev = this.list;
    const old = force ? [] : prev;
    const keep = new Set();
    this.list = videos.map((video, i) => {
      const reuse = old.find((e) => e.video === video && !keep.has(e));
      if (reuse) {
        keep.add(reuse);
        reuse.index = i + 1;
        reuse.started = false;
    ...
    for (const e of prev) if (!keep.has(e)) e.chat.removeAllListeners();
  }

...
class YouTubeLiveChat extends EventEmitter {
  ...
  async start({ apiKey, video, pacing, minIntervalMs, dailyQuota, skipBacklog, share }) {
    this.stop();
  ...
  updateOptions({ pacing, minIntervalMs, dailyQuota, share }) {
    if (!this.info) return;
    this.info.share = Math.max(1, Number(share) || 1);
  ...
  _nextWait(apiMinMs) {
    ...
    // 複数の配信を同時に取得するときは、残りを配信の数で分け合う
    const calls = Math.floor(left / COST_LIST / (info.share || 1));

...
// チャンネル独自の絵文字（:_name: 形式）の画像 URL を集める（操作画面で画像表示するため）
function customEmojis(runs) {
  const map = {};
  for (const r of runs || []) {
    const e = r.emoji;
    if (e?.isCustomEmoji && e.shortcuts?.[0]) {
      const url = e.image?.thumbnails?.at(-1)?.url;
      if (url) map[e.shortcuts[0]] = url;
    }
  }
  return map;
}
...
function parseItem(item) {
  ...
  const base = {
    ...
    emojis: customEmojis(r.message?.runs),
...
module.exports = { YouTubeWebChat, parseItem, extractJson, runsToText, parseAmount, customEmojis };
